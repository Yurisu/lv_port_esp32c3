...
void GC9A01_flush(lv_disp_drv_t * drv, const lv_area_t * area, lv_color_t * color_map)
{
	...
	/*Column addresses with offset*/
	GC9A01_send_cmd(0x2A);				//0x2A
	data[0] = ((area->x1 + GC9A01_X_OFFSET) >> 8) & 0xFF;
	data[1] = (area->x1 + GC9A01_X_OFFSET) & 0xFF;
	data[2] = ((area->x2 + GC9A01_X_OFFSET) >> 8) & 0xFF;
	data[3] = (area->x2 + GC9A01_X_OFFSET) & 0xFF;
	GC9A01_send_data(data, 4);

	/*Page addresses with offset*/
	GC9A01_send_cmd(0x2B);				//0x2B
	data[0] = ((area->y1 + GC9A01_Y_OFFSET) >> 8) & 0xFF;
	data[1] = (area->y1 + GC9A01_Y_OFFSET) & 0xFF;
	data[2] = ((area->y2 + GC9A01_Y_OFFSET) >> 8) & 0xFF;
	data[3] = (area->y2 + GC9A01_Y_OFFSET) & 0xFF;
	GC9A01_send_data(data, 4);
...
static void GC9A01_set_orientation(uint8_t orientation)
{
    ...
    const char *orientation_str[] = {
        "PORTRAIT-0", "PORTRAIT_INVERTED-1", "LANDSCAPE-2", "LANDSCAPE_INVERTED-3"
    };
