...
class BLEConnectionManager:
    ...
    async def connect_for_mode(self, address, mode, max_retries=MAX_RETRIES, retry_delay=RETRY_DELAY):
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                # 统一处理所有连接错误
                print(f"✗ 连接错误: {e}")
                if not isinstance(e, (BleakError, asyncio.TimeoutError)):
                    import traceback
                    traceback.print_exc()
    ...
    def _find_nus_characteristics(self):
        ...
        Returns:
            (tx_characteristic, rx_characteristic): 返回TX和RX特征对象
        """
    ...
    def _get_characteristic_properties(self, characteristic):
        """
        获取特征属性
        ...
        properties = []
        try:
            if hasattr(characteristic, 'properties'):
                props_str = str(characteristic.properties)
    ...
    async def configure_rx_characteristic(self):
        ...
        try:
            ...
            _, rx_characteristic = self._find_nus_characteristics()
            ...
            # 检查RX特征是否支持写入
            print(f"\n检查RX特征属性...")
            properties = self._get_characteristic_properties(rx_characteristic)

            props_str = "/".join(properties) if properties else "未知"
            print(f"  RX特征支持: {props_str}")

            if '写' in props_str or '写(无响应)' in props_str:
                print(f"\n✓ RX特征已就绪，可以发送数据!")
                return True
            else:
                print(f"✗ RX特征不支持写入操作")
    ...
    async def list_services(self):
        ...
        try:
            ...
            for service in services:
                ...
                for char in service.characteristics:
                    properties = self._get_characteristic_properties(char)
                    props_str = "/".join(properties) if properties else "未知"
    ...
    async def enable_ffe2_notification(self):
        ...
        try:
            ...
            properties = self._get_characteristic_properties(tx_characteristic)
            props_str = "/".join(properties) if properties else "未知"
...
async def main():
    ...
    try:
        ...
        await manager.disconnect()  # 确保无论如何都断开连接
