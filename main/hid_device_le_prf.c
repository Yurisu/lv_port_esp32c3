...
static uint16_t battery_level_handle = 0; // 初始化为 0
...
///            esp_ble_set_encryption(param->connect.remote_bda, ESP_BLE_SEC_ENCRYPT_NO_MITM);
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT: {
            if (param->add_attr_tab.num_handle == BAS_IDX_NB &&
                ...
                //yuri
                battery_level_handle = incl_svc.start_hdl+2;
                ESP_LOGI(HID_LE_PRF_TAG, "Battery Level Handle: 0x%04X", battery_level_handle);
                                ...
                                esp_ble_gatts_cb_param_t *param)
{
    ...
    if (event == ESP_GATTS_REG_EVT) {
        if (param->reg.status == ESP_GATT_OK) {
            ...
            //yuri
            ESP_LOGI(HID_LE_PRF_TAG, "ESP_GATTS_REG_EVT, app_id %04x, gatts_if %d",
                    param->reg.app_id,
                    gatts_if);
        } else {
